...
namespace JiraReporter
{
    public class HealthInspector
    {
        ...
        public HealthInspector(JiraReport report)
        {
            Report = report;
        }
        ...
        public Health GetMonthHealth(double allocatedHours, double totalTimeWorked)
        {
            ...
            var workedDays = SummaryHelpers.GetWorkingDays(DateTime.Now.ToOriginalTimeZone(Report.OffsetFromUtc).StartOfMonth(), DateTime.Now.ToOriginalTimeZone(Report.OffsetFromUtc).AddDays(-1), Policy.MonthlyOptions.Months);
            var workedPerDay = totalTimeWorked / workedDays;
            var monthWorkingDays = SummaryHelpers.GetWorkingDays(DateTime.Now.ToOriginalTimeZone(Report.OffsetFromUtc).StartOfMonth(), DateTime.Now.ToOriginalTimeZone(Report.OffsetFromUtc).EndOfMonth(), Policy.MonthlyOptions.Months);
