...
namespace JiraReporter.Services
{
    class DayLogLoader
    {
        ...
        private void AddIssueToDaylog(JiraDayLog dayLog, IssueDetailed issue)
        {
            ...
            currentIssue.Entries = issue.Entries.Where(e => e.AuthorFullName == _author.Name && e.StartedAt.ToOriginalTimeZone(_context.OffsetFromUtc).Date >= _date && e.StartedAt.ToOriginalTimeZone(_context.OffsetFromUtc) < _date.AddDays(1)).ToList(); 
        ...
        private void SetSubtaskEntries(IssueDetailed issue)
        {
            ...
            {
                subtask.Entries = subtask.Entries.Where(e => e.AuthorFullName == _author.Name && e.StartedAt.ToOriginalTimeZone(_context.OffsetFromUtc).Date >= _date && e.StartedAt.ToOriginalTimeZone(_context.OffsetFromUtc) < _date.AddDays(1)).ToList();
